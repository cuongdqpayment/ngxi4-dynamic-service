...
export class DynamicFormMobilePage {
  ...
  * @param i 
  */
  onClickCheck(sheetName, row, i) {

    //thay đổi giá trị check cho nút check này
    //console.log(sheetName, row, i);

    if (row.isChecked) { // nếu chọn dòng này thì bỏ các dòng khác đi
      // lấy dòng này làm header và lấy dữ liệu từ đây trở xuống nhé
      //console.log(this.excelPreviewer.dataSources[sheetName]);
      let arr = this.excelPreviewer.dataSources[sheetName].filteredData;
      arr.forEach((el, idx) => {
        if (el.isChecked && idx !== i) el.isChecked = false // reset các chọn lựa trước đó
      });

      // đồng thời copy dữ liệu của dòng này theo từng cột lấy làm tên để hiển thị lên trên
      for (let key in row) {
        let el = this.excelPreviewer.displayedColumns[sheetName].find(x => x.key === key);
        if (el) el.name = row[key];
      }

      this.excelPreviewer.sheetCtrls[sheetName].startIndex = i + 1; // chọn từ dòng số

    } else {

      this.excelPreviewer.sheetCtrls[sheetName].startIndex = 0;
      // nếu chọn loại bỏ thì trả về trạng thái ban đầu
      for (let key in row) {
        let el = this.excelPreviewer.displayedColumns[sheetName].find(x => x.key === key);
        if (el) el.name = key;
      }
   ...
   */
  fileUploadEvent(evt) {
    // truyền file lên kiểu jsonData hoặc formData
    if (!evt.target) { return; }
    if (!evt.target.files) { return; }
    if (evt.target.files.length !== 1) { return; }
    const file: File = evt.target.files[0]; // đây là đối tượng file cần truyền lên

    //  console.log('File type: ',file);
    // dạng file .csv là cũng chính là định dạng: application/vnd.ms-excel
    // if (file.type !== 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' && file.type !== 'application/vnd.ms-excel') { return; }
    //console.log(file);
    // nếu kiểu formData thì truyền như này
    let form_data: FormData = new FormData();
    let filename = this.apiAuth.Utf8toBase64(file.name) + '_' + Date.now() + '.' + file.name.split('.').pop();
    //console.log(filename,file.lastModified);
    let key = 'excel_upload';

    form_data.append('save_to_dish', 'NO'); // Trạng thái không cần ghi xuống đĩa == 'NO' thì không cần ghi xuống đĩa
    form_data.append(key, file, filename);  // key file upload leen

    form_data.append("options_" + key, JSON.stringify({ // tùy chọn gốc của file đẩy lên
      origin: file.name, //tên file gốc của nó
      type: file.type,
      size: file.size,
      file_date: file.lastModified
    }));

    this.apiCommons.showLoader('Xử lý dữ liệu file upload');

    this.apiAuth.postDynamicFormData(environment.SERVER_PUBLIC + "/excel-to-json", form_data) // không cần bảo mật
      .then(resp => {

        this.apiCommons.hideLoader();
        //console.log('resp', resp);
        this.excelDataSource = resp;
  ...
  /**
   * Cho cửa sổ người dùng chọn cột dữ liệu cho đối tượng import vào
   * @param sheetName 
   */
  clickMakeDataList(sheetName) {

    let colsOptions = [];
    this.excelPreviewer.displayedColumns[sheetName].forEach(el => {
      colsOptions.push({
        name: el.name ? el.name : el.key ? el.key : el,
        value: el.key ? el.key : el
      })
    });
    ...
    this.dynamicForm.items.forEach(el => {
      if (el.key) {
        itemsObject.push({
          ...
          key: el.key,
          name: el.name,
          hint: 'Chọn ' + el.name + ' là bắt buộc',
          // tất cả các cột dữ liệu lấy được {key,name}
          options: colsOptions,
          validators: el.validators ? [{ required: true }] : undefined
        })
      }
    })
    ...
    let form = {
      title: 'GÁN DỮ LIỆU'
      , buttons: [
        { color: 'success', sheet: sheetName, command: 'SAVE-DATA', icon: 'save', next: 'CALLBACK' }
        , { color: 'danger', icon: 'close', next: 'CLOSE' }
      ]
      , items: [
        { type: 'title', name: 'Gán cột với trường' }
      ]
    }

    form.items = form.items.concat(itemsObject); // cộng thêm mảng gán
  ...
  processCallback = function (res) {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      if (res.error) {
        //Lỗi thực thi từ form động
        this.apiCommons.presentAlert('Lỗi:<br>' + (res.message ? res.message : "Error Unknow: " + JSON.stringify(res.error, null, 2)));

      } else if (res.ajax) {
        //gọi để thay đổi dữ liệu trên form
        //ta không cần refresh trang như AJAX

      } else if (res.response_data) {//next="CALLBACK", url="http://..." [,token: true | wzI...]
        //Kết quả trả về sau khi post thành công theo url

      } else if (res.json_data) { ////next="CALLBACK" no url

        if (res.button.command === "SAVE-DATA") {

          this.getDataSheet(res.button.sheet, res.json_data);

          resolve({ next: "CLOSE" }); //gọi lệnh đóng form động
        ...
        //dữ liệu trả về là nút lệnh không, không có dữ liệu kèm theo
        //thì thực hiện việc gì đó, theo lệnh này

        //Ví dụ theo sau:
        if (res.button.command === "DO-ANY") {
          //làm việc gì đó
        }
        resolve({ next: "CLOSE" }); //gọi lệnh đóng form động
        //thực thi nhiệm vụ riêng sau khi đóng
        return; //thoát luôn

      } else {
        //Kết quả trả về là không có dữ liệu gì định nghĩa trước, nên ta làm việc khác

      }

      //các trường hợp còn lại thì tự động đóng form
      resolve({ next: "CLOSE" }); //do nó không có next_data nên sẽ không trả về return close popup

      //ví dụ muốn form kia tiếp tục thì gửi next="NEXT"
      //với next_data.response_data:newForm...->
  ...
  /**
  * Hàm này được gọi khi người dùng đã gán cột của sheet được chọn với
  * trường đối tượng cần tạo
  * 
  * Kết quả là trả về một mảng dữ liệu đầy đủ được import từ sheet của excel
  * 
  * @param sheetName 
  * @param obj 
  */
  getDataSheet(sheetName: string, obj: any) {
    // tạo danh sách trống
    this.jsonImportList = [];

    this.excelDataSource[sheetName].forEach((el, idx) => {

      // Lấy tất cả dữ liệu từ dòng số x được check chọn
      if (!this.excelPreviewer.sheetCtrls[sheetName].startIndex
        || idx >= this.excelPreviewer.sheetCtrls[sheetName].startIndex) {
        ...
        // thay lệnh trên bằng kiểm soát unique, validators, null, và undefined
        this.pushFormListUnique(newObject);

      }
    });

    // Dữ liệu cuối cùng lấy được
    console.log('ouput',this.jsonImportList);

  }

  /**
     * Kiểm tra item và xác nhận hợp lệ của dữ liệu mới chèn vào mãng
     * @param item 
     */
  validatorObject(item) {

    let invalid = false; // kiểm soát hợp lệ ràng buộc dữ liệu
    // duyệt tất cả các trường yêu cầu validator rồi thực hiện valid như form
    this.dynamicForm.$validators.some(el => {

      let value = item[el.key];
      //console.log('Lay gia tri de kiem chung', value, el.validators);
      ...
      let control = new FormControl(value, validatorFns);
      invalid = control.invalid;

      // thoát vòng some nếu không hợp lệ = true nó sẽ tự thoát luôn
      return invalid;

    })

    // kiểm chứng đã valid xong

    if (!invalid) {
      // Đã validate hết dữ liệu

      if (!this.dynamicForm.$uniques || !this.dynamicForm.$uniques.length) {
        // nếu không có unique thì trả về dữ liệu hợp lệ
        return item;
      }

      let isUnique = false;
      // duyệt tất cả các trường yêu cầu unique
      this.dynamicForm.$uniques.some(uk => {
        // tìm bảng ghi bị trùng mà tồn tại thì không ghi thêm vào
        let uniqueIndex = this.jsonImportList.findIndex(x => x[uk] === item[uk]);
        if (uniqueIndex < 0) {
          // xác nhận unique hợp lệ
          return false;
        } else {
          // console.log('Dữ liệu bị trùng', item[uk], item);
          isUnique = true;
          return true; // trả ngay thoát vòng lặp
        }
      });

      // nếu không có unique hoặc duyệt hết mà không vi phạm thì trả item
      if (!isUnique) return item;

    } else {
      // Bảng ghi này không hợp lệ trường dữ liệu nào đó
      // console.log('Bảng ghi không hợp lệ', item);
    }
  ...
  /**
   * Hàm đẩy giá trị unique, validators, value ở unique
   * @param itemOrItems là mãng dữ liệu cần thêm vào dynamicForm.items
   */
  pushFormListUnique(itemOrItems) {
    // this.dynamicForm.$uniques = this.dynamicForm.$objectForm.items.filter(x=>x['unique']).map(o => o['key']);
    // this.dynamicForm.$validators = this.dynamicForm.$objectForm.items.filter(x=>x['validators']);
    // console.log('Danh sách trường unique', itemOrItems);
    // console.log('Danh sách trường unique',itemOrItems,this.dynamicForm.$uniques, this.dynamicForm.$validators);
    ...
    if (Array.isArray(itemOrItems)) {
      ...
      itemOrItems.forEach(el => {
        // console.log('el', el);
        let item = this.validatorObject(el);
        if (item) this.jsonImportList.push(item);
      })

    } else if (typeof itemOrItems === 'object') {

      let item = this.validatorObject(itemOrItems);
      if (item) this.jsonImportList.push(item);
  ...
  /**
   * Xem trước các dòng dữ liệu mẫu 
   * Mặt định là xem 20 dòng đầu tiên
   */
  previewStartRow(sheetName, startRow?: number, numOfRows?: number) {
    // Định nghĩa dữ liệu hiển thị
    this.excelPreviewer.dataSources[sheetName] = new MatTableDataSource<any>(this.excelDataSource[sheetName].slice((startRow ? startRow : 0), (startRow ? startRow + (numOfRows ? numOfRows : 20) : (numOfRows ? numOfRows : 20))));
  }

  /**
   * Chuyển đổi dữ liệu dataSource thành dữ liệu có thể hiển thị và lọc được
   */
  previewExcelData() {

    //console.log(this.excelDataSource);

    if (this.excelDataSource && (typeof this.excelDataSource === 'object')) {
      // Khởi tạo các đối tượng và biến ban đầu để hiển thị excel trước
      this.excelPreviewer = {} // tạo đối tượng ban đầu
      this.excelPreviewer.tables = []; // tạo các bảng
      this.excelPreviewer.sheetCtrls = {}; // tạo các biến điều khiển
      this.excelPreviewer.displayedColumns = {}; // tạo đối tượng ghi key và name
      this.excelPreviewer.columnsToDisplay = {}; // tạo đối tượng hiển thị từng cột
      this.excelPreviewer.dataSources = {} // tạo đối tượng ghi dữ liệu từ excel đẩy vào

      for (let keySheet in this.excelDataSource) {
        // duyệt từng sheet
        // lấy tên bảng
        this.excelPreviewer.tables.push(keySheet);

        // Tạo trường sheet chứa các cột có dữ liệu
        Object.defineProperty(
          this.excelPreviewer.displayedColumns
          , keySheet
          , { value: [], writable: true, enumerable: true, configurable: true })

        // Định nghĩa các tham số để điều khiển sheet đó
        Object.defineProperty(
          this.excelPreviewer.sheetCtrls
          , keySheet
          , { value: {}, writable: true, enumerable: true, configurable: true })

        // các cột hiển thị trên bảng của sheet ghi riêng (có thể ít hơn hoặc nhiều hơn)
        Object.defineProperty(
          this.excelPreviewer.columnsToDisplay
          , keySheet
          , { value: [], writable: true, enumerable: true, configurable: true })

        // Định nghĩa dataSource 5 dòng xem trước
        Object.defineProperty(
          this.excelPreviewer.dataSources
          , keySheet
          , {
            value: null//new MatTableDataSource<any>(this.excelDataSource[keySheet].slice(0, 5))
            , writable: true, enumerable: true, configurable: true
          });

        // xem từ dòng số 0 - 20
        this.previewStartRow(keySheet, 0, 20);


        // lấy tất cả các dòng của 1 sheet
        let rows = this.excelDataSource[keySheet];
        let cols = {}
        // duyệt tất cả các dòng
        rows.forEach((el, idx) => {
          // duyệt tất cả các cột
          for (let keyCell in el) {
            if (!cols[keyCell]) {
              Object.defineProperty(cols, keyCell, { value: 'x', writable: true, enumerable: true, configurable: true })
              this.excelPreviewer.displayedColumns[keySheet].push({ key: keyCell });
              this.excelPreviewer.columnsToDisplay[keySheet].push(keyCell);
            }
          }
        });

        // sắp xếp thứ tự cột và ghi ra mảng
        //this.apiCommons.or
    ...
    //console.log(this.excelPreviewer);
    // tự hiển thị nút check luôn nhé
    this.excelPreviewer.tables.forEach((sheetName, idx) => {

      this.excelPreviewer.sheetCtrls[sheetName].isViewIndex = !this.excelPreviewer.sheetCtrls[sheetName].isViewIndex;

      if (this.excelPreviewer.sheetCtrls[sheetName].isViewIndex) {
        this.excelPreviewer.columnsToDisplay[sheetName].unshift('STT');
      } else {
        this.excelPreviewer.columnsToDisplay[sheetName].shift('STT');
      }
